...
class FatcaUSW9TaxPayerDetailsUseCase
    extends BaseUseCase<LocalError, FatcaUSW9TaxPayerDetailsUseCaseParams, bool> {
  @override
  Future<Either<LocalError, bool>> execute({required FatcaUSW9TaxPayerDetailsUseCaseParams params}) {
    ...
    if (taxPayerType!.isEmpty) {
      ...
      if (socialSecurityNumber!.isEmpty) {
        return Left(AppError(
            error: ErrorInfo(message: ''), type: ErrorType.INVALID_SECURITY_NUMBER, cause: Exception()));
      }
    } else if (isEmployer) {
      if (employerIdNumber!.isEmpty) {
        return Left(
            AppError(error: ErrorInfo(message: ''), type: ErrorType.INVALID_EMPLOYER_ID, cause: Exception()));
      }
    }
    return Right(true);
  }
