...
class FatcaUSRelevantW8AddressDetailsUseCase
    extends BaseUseCase<LocalError, FatcaUSRelevantW8AddressDetailsUseCaseParams, bool> {
  @override
  Future<Either<LocalError, bool>> execute({required FatcaUSRelevantW8AddressDetailsUseCaseParams params}) {
  ...
  Either<AppError, bool> verify() {
    if (permanentAddress!.isEmpty) {
      return Left(AppError(
          error: ErrorInfo(message: ''), type: ErrorType.INVALID_PERMANENT_ADDRESS_LINE, cause: Exception()));
      ...
      if (differentAddress!.isEmpty) {
        return Left(AppError(
            error: ErrorInfo(message: ''), type: ErrorType.DIFFERENT_ADDRESS_LINE, cause: Exception()));
      } else if (differentCountry!.isEmpty) {
        return Left(
            AppError(error: ErrorInfo(message: ''), type: ErrorType.DIFFERENT_COUNTRY, cause: Exception()));
      } else if (differentState!.isEmpty) {
        return Left(
            AppError(error: ErrorInfo(message: ''), type: ErrorType.DIFFERENT_STATE, cause: Exception()));
      } else if (differentCity!.isEmpty) {
        return Left(
            AppError(error: ErrorInfo(message: ''), type: ErrorType.DIFFERENT_CITY, cause: Exception()));
      } else if (differentPostCode!.isEmpty) {
        return Left(
            AppError(error: ErrorInfo(message: ''), type: ErrorType.DIFFERENT_POSTCODE, cause: Exception()));
      }
    }
    return Right(true);
  }
